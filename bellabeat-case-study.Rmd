...
average_daily_activity <- daily_activity %>%
  summarise(average_steps = mean(total_steps), average_sedentary_minutes = mean(sedentary_minutes), average_active_minutes = mean(mean(very_active_minutes), mean(fairly_active_minutes), mean(lightly_active_minutes)), average_sedentary_minutes= mean(sedentary_minutes))
...
```{r}
ggplot(data=daily_activity, aes(x=weekday))+geom_bar(fill='orange')+ labs(title='Frequency of Weekdays')
  
```
```{r}
ggplot(data=daily_sleep, aes(x=weekday)) + geom_bar(fill="orange")
```
```{r}
